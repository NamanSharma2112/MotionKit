...
// ── TYPES ──────────────────────────────────────────────────────────────────
type BunnyState = "sleeping" | "awake" | "surprised" | "success";
...
// Each state returns two <rect> or <circle> or <path> descriptors
// sleeping  → two small horizontal rects (closed eyes)
// awake     → two small circles (open eyes)
// surprised → two larger circles (wide eyes)
// success   → one rect + one arc (wink)
type EyeShape =
...
const EYES: Record<BunnyState, EyeSet> = {
  sleeping: {
    left:  { type: "rect", x: 5.777, y: 19.766, w: 5,   h: 1.4, rx: 0.7 },
    right: { type: "rect", x: 17.378, y: 19.766, w: 5,   h: 1.4, rx: 0.7 },
  },
  awake: {
    left:  { type: "circle", cx: 8.277,  cy: 20.466, r: 1.5 },
    right: { type: "circle", cx: 19.878, cy: 20.466, r: 1.5 },
  },
  surprised: {
    left:  { type: "circle", cx: 8.277,  cy: 20.466, r: 2.2 },
    right: { type: "circle", cx: 19.878, cy: 20.466, r: 2.2 },
  },
  success: {
    left:  { type: "rect", x: 5.777, y: 19.766, w: 5, h: 1.4, rx: 0.7 }, // closed (wink)
    right: { type: "circle", cx: 19.878, cy: 20.466, r: 1.5 },             // open
  },
};

// ── SINGLE EYE ─────────────────────────────────────────────────────────────
function Eye({ shape, color }: { shape: EyeShape; color: string }) {
  if (shape.type === "rect") {
    return (
      <motion.rect
        fill={color}
...
// ── FLOATING Z ─────────────────────────────────────────────────────────────
// Each Z is an SVG path drawn as a "Z" shape using lines
const Z_DEFS = [
  { d: "M32 5 L36 5 L32 9 L36 9",   strokeW: 1.5, delay: 0,   tx: 4, ty: -14, scale: [0.7, 1.0] },
  { d: "M34 3 L37 3 L34 6 L37 6",   strokeW: 1.3, delay: 1,   tx: 6, ty: -16, scale: [0.6, 0.9] },
  { d: "M36 1 L38.5 1 L36 3.5 L38.5 3.5", strokeW: 1.1, delay: 2, tx: 8, ty: -18, scale: [0.5, 0.8] },
];
...
function FloatingZ({
  ...
  return (
    <motion.path
      d={def.d}
      stroke={color}
      strokeWidth={def.strokeW}
      strokeLinecap="round"
      strokeLinejoin="round"
      fill="none"
      initial={{ opacity: 0, x: 0, y: 0, scale: def.scale[0] }}
      animate={{
        opacity: [0, 1, 1, 0],
        x: [0, def.tx],
        y: [0, def.ty],
        scale: [def.scale[0], def.scale[1]],
      }}
      transition={{
        duration: 3,
        delay: def.delay,
...
function Ear({
  ...
  return (
    <motion.g
      transform={`rotate(${rotateDeg}, ${originX}, 10) translate(${translateX}, 1)`}
      // ear wiggle when awake
      animate={
        sleeping
          ? {
          ...
          : {
              rotate: [0, direction === "left" ? -6 : 6, 0, direction === "left" ? 4 : -4, 0],
            }
      }
...
export default function BunnyIcon({
  ...
  const scale = size / 40; // viewBox is 40×40
  ...
  return (
    <motion.svg
      ...
      // whole-body breathe when sleeping
      animate={
        sleeping
          ? { y: [0, 1.5, 0] }
          : { y: 0 }
    ...
    >
      {/* scale inner group to match size */}
      <g transform={`scale(${scale}) translate(${(40 - 40 / scale) / 2 / scale}, 0)`}>
        <g transform="translate(6, 8)">
          ...
          {/* ── Eyes (state-driven) ── */}
          <Eye shape={EYES[state].left}  color={color} />
          <Eye shape={EYES[state].right} color={color} />
        ...
        {sleeping && (
          <g>
            {Z_DEFS.map((def, i) => (
              <FloatingZ key={i} def={def} color={color} />
            ))}
          </g>
        )}
      </g>
...
//   <BunnyIcon size={48} />
//
// State changes (e.g. loading indicator):
//   <BunnyIcon state={isLoading ? "sleeping" : "success"} />
//
// Custom color (matches your theme):
//   <BunnyIcon color="rgba(200,184,154,0.7)" size={24} />
//
// All states:
//   <BunnyIcon state="sleeping" />   — z's float, ears sway gently, body breathes
//   <BunnyIcon state="awake" />      — eyes open (circles), ears perk up
//   <BunnyIcon state="surprised" />  — wide circle eyes, ear snap
//   <BunnyIcon state="success" />    — wink (one closed, one open)